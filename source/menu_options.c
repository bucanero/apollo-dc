...
static void _draw_OptionsMenu(u8 alpha)
{
    ...
    while ((option_name = menu_options[ind].name))
    {
        ...
        DrawString(MENU_ICON_OFF + MENU_TITLE_OFF + 10, y_off, option_name);

		switch (menu_options[ind].type)
		{
			case APP_OPTION_BOOL:
				c = (*menu_options[ind].value == 1) ? opt_on_png_index : opt_off_png_index;
				DrawTextureCenteredX(&menu_textures[c], OPTION_ITEM_OFF-2, y_off+2, 0, menu_textures[c].width /2, menu_textures[c].height /2, 0xFFFFFF00 | alpha);
				break;
			case APP_OPTION_CALL:
				DrawTextureCenteredX(&menu_textures[footer_ico_x_png_index], OPTION_ITEM_OFF, y_off+4, 0, menu_textures[footer_ico_x_png_index].width /2, menu_textures[footer_ico_x_png_index].height /2, 0xFFFFFF00 | alpha);
				break;
			case APP_OPTION_LIST:
				SetFontAlign(FONT_ALIGN_CENTER);
				DrawFormatString(OPTION_ITEM_OFF, y_off, "< %s >", menu_options[ind].options[*menu_options[ind].value]);
