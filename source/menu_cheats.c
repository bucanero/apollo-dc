...
void Draw_CheatsMenu_Options_Ani(void)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawScrollBar(menu_old_sel[7], list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 30, game_a);
...
void Draw_CheatsMenu_Options(void)
{
	...
	DrawScrollBar(menu_sel, list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 30, 0xFF);
}
...
void Draw_CheatsMenu_View(const char* title)
{
    ...
    DrawScrollBar(menu_sel, nlines, APP_LINE_OFFSET, SCREEN_WIDTH - 30, 0xFF);
}
